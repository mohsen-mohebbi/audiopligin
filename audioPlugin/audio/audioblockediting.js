...
export default class AudioBlockEditing extends Plugin {
	...
	init() {
		...
		if (editor.plugins.has('AudioInlineEditing')) {
			editor.commands.add(
				'audioTypeBlock',
				new AudioTypeCommand(this.editor, 'audioBlock')
			);
	...
	_setupConversion() {
		...
		conversion.for('dataDowncast').elementToElement({
			...
			view: (modelElement, { writer }) =>
				createAudioViewElement(writer, 'audioBlock'),
		});
		...
		conversion
			...
			.add(downcastAudioAttribute(audioUtils, 'audioBlock', 'source'))
		...
		conversion
			...
			.elementToElement({
				...
				model: (viewAudio, { writer }) =>
					writer.createElement('audioBlock', {
		...
		this.listenTo(
			...
			(evt, data) => {
				...
				if (
					...
					data.content =
						writer.createDocumentFragment(blockViewAudios);
				}
			}
		);
	}
