...
import AudioStyleEditing from './audiostyleediting';
...
export default class AudioStyleUI extends Plugin {
    ...
    get localizedDefaultStylesTitles() {
        ...
        return {
            ...
            'Left aligned audio': t( 'صوت تراز شده در راست' ),
            'Centered audio': t( 'صوت در وسط' ),
            'Right aligned audio': t( 'صوت تراز شده در چپ' )
        };
    }

    init() {
        const plugins = this.editor.plugins;
        const toolbarConfig = this.editor.config.get( 'audio.toolbar' ) || [];

        const definedStyles = translateStyles(
            plugins.get( 'AudioStyleEditing' ).normalizedStyles,
    ...
    _createButton( buttonConfig ) {
        ...
        this.editor.ui.componentFactory.add( getUIComponentName( buttonName ), locale => {
            const command = this.editor.commands.get( 'audioStyle' );
    ...
    _executeCommand( name ) {
        this.editor.execute( 'audioStyle', { value: name } );
