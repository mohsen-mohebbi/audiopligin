...
import { downcastVideoAttribute, upcastVideoFigure,upcastVideo,downcastSourcesAttribute } from './converters';
...
export default class VideoBlockEditing extends Plugin {
	...
	init() {
		...
		schema.register('videoBlock', {
			...
			isBlock: true,
			allowWhere: '$block',
			allowAttributes: ['src', 'poster', 'controls','source'],
		});
	...
	_setupConversion() {
		...
		conversion
			...
			.add(downcastVideoAttribute(videoUtils, 'videoBlock', 'source'))

			.add(downcastVideoAttribute(videoUtils, 'videoBlock', 'controls'))

			.add(downcastVideoAttribute(videoUtils, 'videoBlock', 'class'))
